...
function nextProject() {
  //log time til next
  console.log(
    `next project lasts for ${data[currentProject].time} miliseconds`
  );
  currentProject++;
  //fixes test slides
  ...
  loadImg(Screen);
  imgCycle();
  cycler = setInterval(imgCycle, 10000);
}

function StopNext() {
  console.log("stop next func");
...
/* const projectCycle = setInterval(StopNext, data[currentProject].time); */
nextProject();
setTimeout(() => {
  StopNext();
}, data[currentProject].time);
